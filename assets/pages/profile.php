            ...
            <div class="col-md-8 col-sm-11">
                <div class="d-flex flex-column">
                    <div class="d-flex gap-5 align-items-center">
                        <span style="font-size: xx-large; color: #3a3b3c; font-weight: 500;"><?=$profile['first_name']?> <?=$profile['last_name']?></span>
...
<a style="font-family: 'Roboto Slab', serif; background-color: #1ebbd7; color: white; font-weight: 500;" class="btn btn-sm btn-primary"><i class="bi bi-file-post-fill"></i> <?=count($profile_post)?> Posts</a>
<a style="font-family: 'Roboto Slab', serif; background-color: #1ebbd7; color: white; font-weight: 500;" class="btn btn-sm btn-primary <?=count($profile['followers'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#follower_list"><i class="bi bi-people-fill"></i> <?=count($profile['followers'])?> Followers</a>
<a style="font-family: 'Roboto Slab', serif; background-color: #1ebbd7; color: white; font-weight: 500;" class="btn btn-sm btn-primary <?=count($profile['following'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#following_list"><i class="bi bi-person-fill"></i> <?=count($profile['following'])?> Following</a>
...
<?php
   ...
   ?>
<button style="font-family: 'Roboto Slab', serif; background-color: #ee5e68; color: white; font-weight: 500;" class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?=$profile['id']?>' >Unfollow</button>
   
   <?php
}else{
    ?> 
<button style="font-family: 'Roboto Slab', serif; background-color: #1ebbd7; color: white; font-weight: 500;" class="btn btn-sm btn-primary followbtn" data-user-id='<?=$profile['id']?>' >Follow</button>
...
?>
        ...
        <h3 style="color: #3a3b3c; font-weight: 500;" class="border-bottom">Posts</h3>
        <?php
...
<div class="d-flex align-items-center p-2">
  ...
  <div class="input-group p-2 border-top">
                            ...
                            <button style="font-family: 'Roboto Slab', serif; background-color: #1ebbd7; color: white; font-weight: 500;" class="btn btn-outline-primary rounded-0 border-0 add-comment" data-cs="comment-section<?=$post['id']?>" data-post-id="<?=$post['id']?>" type="button"
...
foreach($profile['followers'] as $f){
    ...
    if(checkFollowStatus($f['follower_id'])){
        $fbtn = '<button style="font-family: `Roboto Slab`, serif; background-color: #ee5e68; color: white; font-weight: 500;" class="btn btn-sm btn-danger unfollowbtn" data-user-id='.$fuser['id'].' >Unfollow</button>';
    }else if($user['id']==$f['follower_id']){
        $fbtn='';
    }else{
        $fbtn = '<button style="font-family: `Roboto Slab`, serif; background-color: #1ebbd7; color: white; font-weight: 500;" class="btn btn-sm btn-primary followbtn" data-user-id='.$fuser['id'].' >Follow</button>';
